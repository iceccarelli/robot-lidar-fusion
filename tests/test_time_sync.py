...
def test_match_nearest_camera_frame() -> None:
    ...
    cam1 = CameraFrame(timestamp=1.0, frame_id="cam", image=None, intrinsics={}, metadata={})
    cam2 = CameraFrame(timestamp=2.0, frame_id="cam", image=None, intrinsics={}, metadata={})
    ts.add_camera_frame(cam1)
    ts.add_camera_frame(cam2)
    # LiDAR frame at 1.05 s should match cam1
    lidar = LidarFrame(timestamp=1.05, frame_id="lidar", points_xyz=[], intensities=None, metadata={})
...
def test_match_returns_none_when_offset_too_large() -> None:
    ...
    cam = CameraFrame(timestamp=1.0, frame_id="cam", image=None, intrinsics={}, metadata={})
    ts.add_camera_frame(cam)
    # LiDAR frame 0.2 s away should not match
    lidar_far = LidarFrame(timestamp=1.2, frame_id="lidar", points_xyz=[], intensities=None, metadata={})
